...
Unbxd.setSearch.prototype.defaultOptions = {inputSelector: "#search_query",searchButtonSelector: "#search_button",type: "search",getCategoryId: "",spellCheck: "",spellCheckTemp: "<h3>Did you mean : {{suggestion}}</h3>",searchQueryDisplay: "",searchQueryDisplayTemp: "<h3>Search results for {{query}} - {{numberOfProducts}}</h3>",searchResultContainer: "",searchResultSetTemp: "",isAutoScroll: false,isClickNScroll: false,clickNScrollSelector: "",pageSize: 15,facetMultiSelect: true,facetContainerSelector: "",facetCheckBoxSelector: "",selectedFacetTemp: "{{#each filters}}" + "<ol>" + "<li>" + '<span class="label">{{prepareFacetName @key}}:</span>' + "{{#each this}}" + '<div class="refineSect">{{@key}}<a href="#" class="btn-remove"></a>' + "</div>" + "{{/each}}" + "</li>" + "</ol>" + "{{/each}}",selectedFacetContainerSelector: "",clearSelectedFacetsSelector: "",removeSelectedFacetSelector: "",loaderSelector: "",onFacetLoad: "",onIntialResultLoad: "",onPageLoad: "",sanitizeQueryString: function(q) {
        return q
    },getFacetStats: "",processFacetStats: function(obj) {
    },setDefaultFilters: function() {
    },enableBuckets: false,noOfBuckets: 5,bucketSize: 5,bucketField: "",bucketResultSetTemp: ""};
jQuery.extend(Unbxd.setSearch.prototype, {compiledResultTemp: false,compiledBucketResultTemp: false,compiledSpellCheckTemp: false,compiledSearchQueryTemp: false,compiledFacetTemp: false,compiledSelectedFacetTemp: false,currentNumberOfProducts: 0,totalNumberOfProducts: 0,isLoading: false,params: {query: "*",filters: {},ranges: {},sort: {},categoryId: "",extra: {wt: "json",page: 1,rows: 12}},isHistory: !!(window.history && history.pushState),popped: false,initialURL: "",isHashChange: false,currentHash: "",hashChangeInterval: null,init: function() {
        ...
        if (this.params.categoryId.length > 0) {
            ...
            var cur_url = window.location.hash.substring(1) || window.location.search.substring(1), urlqueryparams = this.getQueryParams(cur_url), decodedParams = this.getQueryParams(this.decode(cur_url)), queryparamcount = Object.keys(urlqueryparams).length, decodedParamscount = Object.keys(decodedParams).length;
        ...
        if (this.options.type == "search") {
            if ("form" in this.input && this.input.form) {
                jQuery(this.input.form).bind("submit", function(e) {
                    ...
                    self.clearFilters().setPage(1).setPageSize(self.options.pageSize);
        ...
        if (this.options.facetContainerSelector.length > 0) {
            jQuery(this.options.facetContainerSelector).delegate(self.options.facetCheckBoxSelector, "change", function(e) {
                ...
                self.setPage(1).callResults(self.paintResultSet, true)
            })
        }
        if (this.options.clearSelectedFacetsSelector.length > 0) {
            jQuery(this.options.clearSelectedFacetsSelector).bind("click", function(e) {
                e.preventDefault();
                self.clearFilters().setPage(1).callResults(self.paintResultSet, true)
            })
        }
        if (this.options.removeSelectedFacetSelector.length > 0) {
            jQuery(this.options.selectedFacetContainerSelector).delegate(this.options.removeSelectedFacetSelector, "click", function(e) {
                e.preventDefault();
                var $t = jQuery(this), name = $t.attr("unbxdParam_facetName"), val = $t.attr("unbxdParam_facetValue"), checkbox_sel = self.options.facetCheckBoxSelector + "[unbxdParam_facetName='" + name + "'][unbxdParam_facetValue='" + val + "']";
                ...
                self.removeFilter(name, val).setPage(1).callResults(self.paintResultSet, true)
            })
        }
        if (this.isHistory) {
        ...
        for (var key in this.params.extra) {
            if (this.params.extra.hasOwnProperty(key)) {
                ...
                } else if (!(this.options.enableBuckets && key == "rows"))
        ...
        return {url: host_path + "?" + url,query: url,host: host_path}
        ...
        var self = this, modifiedCB = callback.bind(self), cb = function(data) {
        ...
        jQuery.ajax({url: urlobj.url,dataType: "jsonp",jsonp: "json.wrf",success: cb.bind(self)})
    },reset: function() {
        ...
        this.params = {query: "*",filters: {},sort: {},ranges: {},categoryId: "",extra: {wt: "json",page: 1,rows: 12}}
    },_processURL: function(url) {
        var obj = typeof url == "string" ? this.getQueryParams(url) : url, params = {query: "",filters: {},sort: {},ranges: {},extra: {wt: "json",page: 1,rows: 12}};
        if ("filter" in obj) {
        ...
        jQuery(this.options.searchQueryDisplay).html(this.compiledSearchQueryTemp({query: obj.searchMetaData.queryParams.q,numberOfProducts: this.options.enableBuckets ? obj.buckets.totalProducts : obj.response.numberOfProducts})).show();
        if (this.options.enableBuckets) {
            var processed = [];
            for (var x in obj.buckets) {
                if (obj.buckets.hasOwnProperty(x)) {
                    if (x === "totalProducts" || x === "numberOfBuckets")
                        continue;
                    processed.push({name: x,numberOfProducts: obj.buckets[x].numberOfProducts,products: obj.buckets[x].products.slice()})
                }
            }
            if (this.getClass(this.options.bucketResultSetTemp) == "Function") {
                this.options.bucketResultSetTemp({buckets: processed})
            } else {
                if (!this.compiledBucketResultTemp)
                    this.compiledBucketResultTemp = Handlebars.compile(this.options.bucketResultSetTemp);
                jQuery(this.options.searchResultContainer).append(this.compiledBucketResultTemp({buckets: processed,query: obj.searchMetaData.queryParams.q,numberOfProducts: this.options.enableBuckets ? obj.buckets.totalProducts : obj.response.numberOfProducts}))
            }
        ...
        if ("error" in obj)
            return;
        var facets = obj.facets, facetKeys = Object.keys(facets), newfacets = [], singlefacet = {}, self = this, facetVal = "", isSelected = false, selectedOnly = [];
        for (var x in facets) {
            singlefacet = {name: self.prepareFacetName(x),facet_name: x,type: facets[x]["type"],selected: [],unselected: []};
        ...
        if (this.options.getFacetStats.length && typeof this.options.processFacetStats == "function" && "stats" in obj) {
            obj.stats[this.options.getFacetStats].values = {min: this.options.getFacetStats in this.params.ranges && this.params.ranges[this.options.getFacetStats].lb != "*" ? this.params.ranges[this.options.getFacetStats].lb : obj.stats[this.options.getFacetStats].min,max: this.options.getFacetStats in this.params.ranges && this.params.ranges[this.options.getFacetStats].ub != "*" ? this.params.ranges[this.options.getFacetStats].ub : obj.stats[this.options.getFacetStats].max};
